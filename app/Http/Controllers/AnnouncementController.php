...
class AnnouncementController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('Create Announcement'))
        {
            ...
            return view('announcement.create', compact('employees', 'branches', 'departments'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('Create Announcement'))
        {
            ...
            foreach($departmentEmployee as $employee)
            {
                ...
                // Notification to All Employee
                $fcmToken = User::where('type', 'hr')->first()->fcm_token;
                $notificationData = [
                    'title' => $request->title,
                    'body' => substr($request->description, 0, 100) . (strlen($request->description) > 100 ? '...' : ''),
                    'fcm_token' => $fcmToken,
                ];
                try {
    ...
    public function edit($announcement)
    {
        if(\Auth::user()->can('Edit Announcement'))
        {
            ...
            if($announcement->created_by == Auth::user()->creatorId())
            {
                $branches    = Branch::where('created_by', \Auth::user()->creatorId())->get();
                $departments = Department::where('created_by', \Auth::user()->creatorId())->get();
                return view('announcement.edit', compact('announcement', 'branches', 'departments'));
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
