...
class SettingsController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'super admin') {
            ...
            // Initialize post array with all form data
            $post = $request->all();
            unset($post['_token']);
            ...
            if ($request->hasFile('logo')) {
                $request->validate([
                    'logo' => 'image|mimes:png,svg,jpg,jpeg|max:2048',
                ]);
                ...
                // Store in storage/app/public/uploads/logo/
            ...
            if ($request->hasFile('landing_logo')) {
                $request->validate([
                    'landing_logo' => 'image|mimes:png,svg,jpg,jpeg|max:2048',
                ]);
                
                $landingLogoFile = $request->file('landing_logo');
                $extension = $landingLogoFile->getClientOriginalExtension();
                $landingLogoName = 'landing_logo.' . $extension;
                
                // Store in storage/app/public/uploads/logo/
                $landingLogoPath = $landingLogoFile->storeAs('uploads/logo', $landingLogoName, 'public');
                $post['landing_logo'] = $landingLogoPath;
            }

            if ($request->hasFile('favicon')) {
                $request->validate([
                    'favicon' => 'image|mimes:png,svg,jpg,jpeg,ico|max:1024',
                ]);
                ...
                // Store in storage/app/public/uploads/logo/
                $faviconPath = $faviconFile->storeAs('uploads/logo', $faviconName, 'public');
                $post['favicon'] = $faviconPath;
            }

            // Handle checkbox inputs - set to 'off' if not present
            $checkboxFields = [
            ...
            if (isset($post['SITE_RTL'])) {
                $arrEnv['SITE_RTL'] = $post['SITE_RTL'];
            }
            if (isset($post['default_language'])) {
                $arrEnv['DEFAULT_LANGUAGE'] = $post['default_language'];
            }
            ...
            // Get current settings to check which fields are valid
            $settings = Utility::settings();
            
            // Save settings to database
            foreach ($post as $key => $data) {
                if ($key !== '_token' && !empty($data)) {
            ...
            try {
                ...
                // Handle artisan command errors silently
            }
