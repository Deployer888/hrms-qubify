...
class DashboardController extends BaseController
{
     ...
     * @OA\Get(
     *     path="/dashboard",
     *     summary="Get the dashboard data for HR or Employee",
     *     description="Returns the dashboard data for the logged-in HR or Employee based on user type.",
     ...
     *         in="query",
     *         required=true,
     *         description="Flag to check if the request is coming from the desktop version.",
     *         @OA\Schema(
     *             type="string",
     *             enum={"1", "0"}
     *         )
     ...
     *                                     @OA\Property(property="textColor", type="string"),
     ...
     *                             )
     *                         ),
     *                         @OA\Property(property="countEmployee", type="integer"),
     *                         @OA\Property(property="countUser", type="integer"),
     *                         @OA\Property(property="countTicket", type="integer"),
     *                         @OA\Property(property="countOpenTicket", type="integer"),
     *                         @OA\Property(property="countCloseTicket", type="integer"),
     *                         @OA\Property(property="notClockIns", type="array", items=@OA\Items(
     *                             type="object",
     *                             properties={
     *                                 @OA\Property(property="id", type="integer"),
     ...
     *                         @OA\Property(property="totalPayee", type="integer"),
     *                         @OA\Property(property="totalPayer", type="integer"),
     *                         @OA\Property(property="meetings", type="array", items=@OA\Items(
     *                             type="object",
     *                             properties={
     *                                 @OA\Property(property="id", type="integer"),
     *                                 @OA\Property(property="title", type="string"),
            ...
            if ($user->device_type == 1) {
                if($userType == 'employee')
                {
                    ...
                    if ($employee->is_probation == 1) {
                        ...
                        $leavesType = LeaveType::leftJoin('leaves', function ($join) use ($employee) {
                        ...
                        $leavesType = LeaveType::leftJoin('employees', function ($join) {
                                            $join->on('employees.user_id', '=', Auth::user()->id);
                                        })
                    ...
                    $leave = [
                        'total' => round((float) $totalAvailableLeaveDays, 2),
                        'applied' => round((float) $totalAppliedLeaveDays, 2),
                        'remain' => round((float) ($totalAvailableLeaveDays - $totalAppliedLeaveDays), 2),
                    ];
