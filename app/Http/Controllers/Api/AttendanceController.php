    ...
    public function attendance(Request $request)
    {
        try {
            ...
            if ($settings['ip_restrict'] == 'on') {
                ...
                if (empty($ip)) {
                    return $this->errorResponse('This IP is not allowed to clock in & clock out.', 200);
                }
            }
            ...
            if ($attendance != null) {
                $attendance->clock_out = $request->time;
                $attendance->clock_out_latitude = $request->latitude;
                $attendance->clock_out_longitude = $request->longitude;
                $attendance->save();
                $attendance['user_id'] = $request->user_id;
                return $this->successResponse($attendance,__('Clock Out successfully.'), 200);
            ...
            $employeeAttendance->employee_name = $emp->name;
            ...
            $employeeAttendance->save();
            $employeeAttendance['user_id'] = $request->user_id;
            return $this->successResponse($employeeAttendance,__('Clock In successfully.'), 200);
