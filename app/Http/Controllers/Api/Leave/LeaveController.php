...
class LeaveController extends BaseController
{
    ...
    public function getLeaveTypes($id, Request $request)
    {
        try {
            $currentYear = date('Y');
            ...
            // Get request type parameter
            $requestType = $request->input('type', 'full'); // default to 'full' if not provided
    
            // Determine gender-based leave types
            $genderBasedLeaves = [];
            if ($employee->gender == 'Female') {
                $genderBasedLeaves[] = 'Maternity Leaves';
            } elseif ($employee->gender == 'Male') {
                $genderBasedLeaves[] = 'Paternity Leaves';
            }
            ...
            if ($employee->is_probation == 1) {
                ...
                // Apply type filter for probation employees
                if (in_array($requestType, ['half', 'short'])) {
                    // For half/short leaves, only keep Sick Leave (Birthday Leave not applicable for half/short)
                    $allowedLeaveTypes = ['Sick Leave'];
                } else {
                    // For full leaves, add gender-based leaves for probation employees
                    $allowedLeaveTypes = array_merge($allowedLeaveTypes, $genderBasedLeaves);
                }
                                    ...
                                    CASE
                                        ...
                                        WHEN leave_types.title = "Birthday Leave" THEN
                                            CASE 
                                                ...
                                                ELSE 0
                                            END
                                        WHEN leave_types.title IN ("Maternity Leaves", "Paternity Leaves") THEN
                                            leave_types.days - COALESCE(SUM(CASE WHEN leaves.status IN ("Approve", "Pending") THEN leaves.total_leave_days ELSE 0 END), 0)
                                        ELSE
                                            leave_types.days - COALESCE(SUM(CASE WHEN leaves.status IN ("Approve", "Pending") THEN leaves.total_leave_days ELSE 0 END), 0)
                                    END AS days
                                    ...
                                    CONCAT(leave_types.title, " (", 
                                        CASE
                                            ...
                                            ELSE
                                                leave_types.days - COALESCE(SUM(CASE WHEN leaves.status IN ("Approve", "Pending") THEN leaves.total_leave_days ELSE 0 END), 0)
                                        END,
                                    " days)") AS title_with_days
                                ')
                            )
                            ->groupBy('leave_types.id', 'leave_types.title', 'leave_types.days')
                            ->get();
            } else {
                $leaveTypesQuery = LeaveType::leftJoin('employees', function ($join) use ($id) {
                                $join->on('employees.user_id', '=', DB::raw("'" . $id . "'"));
                            })
                            ->leftJoin('leaves', function ($join) use ($employee, $currentYear) {
                                $join->on('leave_types.id', '=', 'leaves.leave_type_id')
                ...
                if ($requestType === 'full') {
                    // For full leaves, show all leave types with gender and birthday filtering
                    $leaveTypesQuery->where(function($query) use ($employeeBirthday, $currentDate, $genderBasedLeaves) {
                        // Always exclude the opposite gender's leave
                        $excludeLeaves = ['Maternity Leaves', 'Paternity Leaves'];
                        $excludeLeaves = array_diff($excludeLeaves, $genderBasedLeaves); // Remove the allowed gender-based leave
                        
                        $query->whereNotIn('leave_types.title', $excludeLeaves)
                              ->where(function($subQuery) use ($employeeBirthday, $currentDate) {
                                  $subQuery->where('leave_types.title', '!=', 'Birthday Leave')
                                           ->orWhere(function($birthdayQuery) use ($employeeBirthday, $currentDate) {
                                               $birthdayQuery->where('leave_types.title', '=', 'Birthday Leave')
                                                            ->whereRaw("'" . $employeeBirthday . "' >= '" . $currentDate . "'");
                                           });
                              });
                    });
                } elseif (in_array($requestType, ['half', 'short'])) {
                    // For half/short leaves, only show Paid Leave and Sick Leave (no gender-based leaves)
                    $leaveTypesQuery->whereIn('leave_types.title', ['Paid Leave', 'Sick Leave']);
                }
                
                $leaveTypes = $leaveTypesQuery->select(
                                'leave_types.id',
                                'leave_types.title',
                                DB::raw('
                                    CASE
    ...
    public function changeaction(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'status' => 'required|in:Approve,Reject,Approval',
                'reject_reason' => 'required_if:status,Reject'
            ]);
            ...
            // Update paid leave balance for leave type 3
            ...
            if ($setings['leave_status'] == 1) {
                ...
                if ($employee) {
                    ...
                    try {
                        // Additional receivers for CC
                        $additionalReceivers = [
                            'karan@qubifytech.com',
                        ];
                        $tlid = '';
                        // Add team leader to CC if employee is not a team leader
                        if ($employee->is_team_leader == 0) {
                            $teamLeader = $employee->getTeamLeaderNameAndId();
                            if ($teamLeader) {
            ...
            return response()->json([
                'success' => true,
                'message' => __('Leave status successfully updated.') . ($smtp_error ? ' ' . $smtp_error : ''),
                'data' => [
                    'leave_id' => $leave->id,
                    'status' => $leave->status,
                    'reject_reason' => $leave->reject_reason ?? null,
            ...
            \Log::error("Leave change action error: " . $e->getMessage());
            return response()->json([
                'success' => false,
