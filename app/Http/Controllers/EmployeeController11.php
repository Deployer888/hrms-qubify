...
class EmployeeController extends Controller
{
        ...
        if (\Auth::user()->can('Create Employee')) {
            ...
            $employee = Employee::create(
                [
                    ...
                    'company_doj' => Carbon::parse($request->dob)->format('Y-m-d'),
            ...
            if ($request->hasFile('document')) {
                foreach ($request->document as $key => $document) {
                    ...
                    $docName = Document::where('id', $key)->pluck('name');
    ...
    {
        $teamLeaders = Employee::select('*')->where('branch_id', $request->branchId)->where('department_id',$request->departmentId)->where('is_team_leader', 1)->get();
