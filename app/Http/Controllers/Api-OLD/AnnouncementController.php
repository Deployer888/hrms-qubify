 ...
 *     schema="Announcement",
 *     type="object",
 *     required={"id", "title", "start_date", "end_date", "branch_id", "department_id", "employee_id", "description", "created_by"},
 ...
 */
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->getAllPermissions()->pluck('name')->contains('Create Announcement')) {
            ...
            return response()->json([
                'success' => true,
                'message' => __('Announcement successfully created.')
            ], 201);
        } else {
            return response()->json([
                'success' => false,
    ...
    public function announcementDetail($announcementId)
    {
        if (\Auth::user()->getAllPermissions()->pluck('name')->contains('Create Announcement')) {
            ...
            if ($announcement && $announcement->created_by == Auth::user()->creatorId()) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => __('Permission denied or announcement not found.')
                ], 403);
            }
        } else {
            return response()->json([
    ...
    public function update(Request $request, $announcementId)
    {
        if (\Auth::user()->getAllPermissions()->pluck('name')->contains('Edit Announcement')) {
            ...
            if ($announcement && $announcement->created_by == Auth::user()->creatorId()) {
                ...
                return response()->json([
                    'success' => true,
                    'message' => __('Announcement successfully updated.'),
                ], 200);
            } else {
                return response()->json([
                    'success' => false,
    ...
    public function destroy($announcementId)
    {
        if (\Auth::user()->getAllPermissions()->pluck('name')->contains('Delete Announcement')) {
            ...
            if ($announcement && $announcement->created_by == Auth::user()->creatorId()) {
                ...
                return response()->json([
                    'success' => true,
                    'message' => __('Announcement successfully deleted.'),
                ], 200);
            } else {
                return response()->json([
                    'success' => false,
    ...
    /**
     * @OA\Get(
     *     path="/announcement/getdepartment/{id}",
    ...
    /**
     * @OA\Post(
     *     path="/announcement/getemployee/{id}",
