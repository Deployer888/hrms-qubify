...
class RoleController extends Controller
{
    public function index()
    {
        ...
        {
            $roles = Role::where('created_by', '=', \Auth::user()->creatorId())->where('guard_name','web')->get();
    ...
    public function create()
    {
        ...
        {
            $user = \Auth::user();
            if($user->type == 'super admin' || $user->type == 'company' || $user->type == 'director')
            {
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('Create Role'))
        {
            ...
            else
            {
                ...
                // Create api guard role
                $apiRole             = new Role();
                $apiRole->name       = $name;
                $apiRole->guard_name = 'api'; // Set guard to api
                $apiRole->created_by = \Auth::user()->creatorId();
                $apiRole->save();
                ...
                $validator = Validator::make($request->all(), [
                    'name' => [
                        ...
                        Rule::unique('roles', 'name')
                            ->ignore($role['id'], 'id') // Ignore the current role ID
                            ->where(function ($query) use ($request) {
                                $query->where('created_by', \Auth::user()->creatorId())
                                      ->where('name', '!=', $request->name); // Exclude current name
                            }),
                    ],
                    'permissions' => 'required',
                ]);
    ...
    public function destroy(Role $role)
    {
        if(\Auth::user()->can('Delete Role'))
        {
            if($role->guard_name == 'web'){
                $SecRole = Role::where('name', '=', $role->name)->where('guard_name','api')->first();
            }else{
                $SecRole = Role::where('name', '=', $role->name)->where('guard_name','web')->first();
            }
            $SecRole->delete();
            $role->delete();
