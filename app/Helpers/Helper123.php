...
class Helper
{
    ...
    public static function dynLateTime($shift_time, $time)
    {
        ...
        // If the employee clocked in exactly on time
        return "(On Time)";
    }
    ...
    public static function calculateTotalTimeDifference($attendanceList) {
        ...
        foreach ($attendanceList as $attendance) {
            ...
            if (!empty($attendance->clock_out) && $attendance->clock_out != '00:00:00') {
                ...
                // For active session (no clock-out yet)
                $totalSeconds += ($currentTime - $clockInTime);
            }
        }
        ...
        return sprintf('%d hrs %d mins', $totalHours, $totalMinutes);
        // return sprintf('%d hrs %d mins %d secs', $totalHours, $totalMinutes, $totalSecs);
    }
    ...
    public static function checkLeaveWithTypes($date, $employeeId) {
        ...
        if(!empty($leave)){
            if($leave->leavetype == 'half') {
                if($leave->day_segment == 'morning'){
                    return 'morning halfday';
                }else{
                    return 'afternoon halfday';
                }
            }
            else if($leave->leavetype == 'full'){
                return 'fullday Leave';
            }
            else if($leave->leavetype == 'short'){
                // $currentTime = date("g:i A");
                // if ($currentTime >= $leave->start_time && $currentTime <= $leave->end_time) {
                //     return 'on short leave';
                // }else{
                //     return 'not on short leave';
                // }
                return 'on short leave';
            }
        }
