    ...
    <form action="{{ route('transfer.update', $transfer->id) }}" method="post">
        ...
        <div class="row">
            <div class="form-group col-lg-6 col-md-6">
                ...
                <select name="employee_id" id="employee_id" class="form-control select2" required>
                    @foreach ($employees as $id => $name)
                        <option value="{{ $id }}" @if ($transfer->employee_id == $id) selected @endif>
                            {{ $name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-lg-6 col-md-6">
                ...
                <select name="branch_id" id="branch_id" class="form-control select2">
                    @foreach ($branches as $id => $name)
                        <option value="{{ $id }}" @if ($transfer->branch_id == $id) selected @endif>
                            {{ $name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-lg-6 col-md-6">
                ...
                <select name="department_id" id="department_id" class="form-control select2">
                    @foreach ($departments as $id => $name)
                        <option value="{{ $id }}" @if ($transfer->department_id == $id) selected @endif>
                            {{ $name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group col-lg-12">
                ...
                <textarea name="description" id="description" class="form-control" placeholder="{{ __('Enter Description') }}">{{ $transfer->description }}</textarea>
            </div>
            <div class="col-12">
