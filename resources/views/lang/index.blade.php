...
@section('action-button')
    <div class="all-button-box row d-flex justify-content-end">
        <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
            <a href="#" data-url="{{ route('create.language') }}" class="btn btn-xs btn-white btn-icon-only width-auto"
        ...
        @if ($currantLang != (!empty(env('default_language')) ? env('default_language') : 'en'))
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#!" class="btn btn-xs btn-icon icon-left btn-danger width-auto" data-toggle="tooltip"
                    data-original-title="{{ __('Delete') }}"
                    data-confirm="{{ __('Are You Sure?') . ' | ' . __('This action can not be undone. Do you want to continue?') }}"
                    data-confirm-yes="document.getElementById('delete-form-{{ $currantLang }}').submit();">
                    <i class="fas fa-trash"></i> {{ __('Delete') }}
                </a>
                <form action="{{ route('lang.destroy', $currantLang) }}" method="POST" id="delete-form-{{ $currantLang }}">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
