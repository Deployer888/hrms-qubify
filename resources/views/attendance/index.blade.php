...
<div class="container-fluid px-3">
                            ...
                            @if($requestType == 'daily')
                                ...
                                @foreach ($attendanceEmployee as $attendance)
                                    <tr>
                                        ...
                                        <td><span class="time-badge">{{ $attendance->clock_in != '00:00' ? \Auth::user()->timeFormat($attendance->clock_in) : '00:00' }}</span></td>
                                        <td><span class="time-badge">{{ $attendance->clock_out != '00:00' ? \Auth::user()->timeFormat($attendance->clock_out) : '00:00' }}</span></td>
                                        <td>{{ $attendance->employee->shift_start ?? 'N/A' }}</td>
                                        <td>
                                            <span class="time-badge {{ $attendance->total_rest == '00:00' ? 'late-time' : 'rest-time' }}">
                                                {{ Helper::convertTimeToMinutesAndSeconds($attendance->total_rest == '00:00' ? $attendance->late : $attendance->total_rest) }}{{ $attendance->total_rest == '00:00' ? ' (Late)' : ' (Rest)' }}
                                            </span>
                                        </td>
                                    </tr>
                                @endforeach
                                                ...
                                                <tbody>
                                                    @foreach($attEmp['attendance'] as $attendance)
                                                        <tr>
                                                            <td>{{ date('d-m-Y', strtotime($attendance['date'])) }}</td>
                                                            <td><span class="status-present">{{ $attendance['status'] }}</span></td>
                                                            <td><span class="time-badge">{{ $attendance['clock_in'] != '00:00' ? date('H:i', strtotime($attendance['clock_in'])) : '00:00' }}</span></td>
                                                            <td><span class="time-badge">{{ $attendance['clock_out'] != '00:00' ? date('H:i', strtotime($attendance['clock_out'])) : '00:00' }}</span></td>
                                                            <td>{{ $attendance['employee']['shift_start'] ?? 'N/A' }}</td>
                                                            <td>
                                                                <span class="time-badge {{ $attendance['total_rest'] == '00:00' ? 'late-time' : 'rest-time' }}">
                                                                    {{ Helper::convertTimeToMinutesAndSeconds($attendance['total_rest'] == '00:00' ? $attendance['late'] : $attendance['total_rest']) }}{{ $attendance['total_rest'] == '00:00' ? ' (Late)' : ' (Rest)' }}
                                    ...
                                    @forelse ($employee->attendance ?? [] as $key=>$attendance)
                                        <tr class="{{ $key >= 5 ? 'collapsible-rows employee-' . $employee->id : '' }}">
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    <span class="time-badge late-time">{{Helper::dynLateTime($employee->shift_start??'09:00',$attendance->clock_in)}}</span>
                                                @endif
                                            </td>
                                            @if ((Gate::check('Edit Attendance') || Gate::check('Delete Attendance')) && \Auth::user()->type != 'employee')
                                            <td>
                                                <div class="action-buttons">
                                                    @if($attendance->clock_out != '00:00')
                                                    ...
                                                    @can('Delete Attendance')
                                                        <button class="action-btn btn-delete" 
                                                                onclick="deleteAttendance({{ $attendance->id }}, '{{ $employee->name ?? \Auth::user()->name }}', '{{ $attendance->date }}')"
    ...
    <script>        
        ...
        function testFunctionality() {
            ...
            console.log('Delete buttons found:', $('.delete-attendance').length);
    ...
    <script>
        ...
        $(window).on('load', function() {
            console.log('Window loaded, setting up delete handlers');
            console.log('Delete buttons found:', $('.delete-attendance').length);
            
            // Test if buttons exist and are clickable
            $('.delete-attendance').each(function(i) {
                console.log('Button ' + i + ':', this, 'ID:', $(this).data('attendance-id'));
            });
            
            // Add a test button to verify JavaScript is working
            if ($('.delete-attendance').length > 0) {
                $('body').prepend('<button id="test-btn" style="position:fixed;top:10px;right:10px;z-index:9999;background:red;color:white;padding:10px;">TEST DELETE</button>');
                $('#test-btn').on('click', function() {
                    alert('JavaScript is working! Found ' + $('.delete-attendance').length + ' delete buttons');
                    $('.delete-attendance').first().trigger('click');
                });
            }
            
            // Simple direct click handler
            $('.delete-attendance').off('click').on('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                const $btn = $(this);
                const attendanceId = $btn.attr('data-attendance-id');
                const employeeName = $btn.attr('data-employee-name');
                const attendanceDate = $btn.attr('data-attendance-date');
                
                console.log('Attendance ID:', attendanceId);
                console.log('Employee Name:', employeeName);
                
                if (!attendanceId) {
                    alert('Error: No attendance ID found');
                    return;
                }
                
                // Simple confirmation
                if (confirm('Are you sure you want to delete this attendance record for ' + employeeName + '?')) {
                    ...
                    if (form) {
                        ...
                        console.log('Form not found, trying AJAX...');
                        // AJAX fallback
                        $.ajax({
                            url: '{{ url("attendanceemployee") }}/' + attendanceId,
                            type: 'DELETE',
...
<script>
    $(document).ready(function() {
        ...
        // Enhanced button interactions (exclude delete buttons)
        $(document).on('click', '.action-btn:not(.delete-attendance)', function() {
            var $btn = $(this);
