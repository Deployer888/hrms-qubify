    ...
    <div class="all-button-box row d-flex justify-content-end">
        @can('Create Complaint')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="{{ route('complaint.create') }}" class="btn btn-xs btn-white btn-icon-only width-auto"
                    data-ajax-popup="true" data-title="{{ __('Create New Complaint') }}">
                                            ...
                                            <td class="text-right action-btns">
                                                @can('Edit Complaint')
                                                    <a href="#"
                                                        ...
                                                        data-title="{{ __('Edit Complaint') }}" class="edit-icon"
                                                ...
                                                @can('Delete Complaint')
                                                    <a href="#" class="delete-icon" data-toggle="tooltip"
                                                        ...
                                                        data-confirm-yes="document.getElementById('delete-form-{{ $complaint->id }}').submit();"><i
                                                            class="fas fa-trash"></i></a>

                                                    <form action="{{ route('complaint.destroy', $complaint->id) }}"
                                                        method="POST" id="delete-form-{{ $complaint->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                            @endif
