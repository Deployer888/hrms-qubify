        ...
        <div class="all-button-box row d-flex justify-content-end">
            @if (\Auth::user()->type == 'super admin')
                <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                    <a href="#" data-url="{{ route('user.create') }}" data-size="xl" data-ajax-popup="true"
                        data-title="{{ __('Create New Company') }}" class="btn btn-xs btn-white btn-icon-only width-auto">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    ...
                                    @can('Edit User')
                                        <a href="#" data-url="{{ route('user.edit', $user->id) }}"
                                            class="dropdown-item text-sm" data-ajax-popup="true" data-bs-toggle="tooltip"
                                            data-title="{{ __('Edit User') }}"
                                            title="{{ __('Edit User') }}">{{ __('Edit') }}</a>
                                    @endcan
                                    @can('Delete User')
                                        <a href="#" class="dropdown-item text-sm" data-bs-toggle="tooltip"
                                            title="{{ __('Delete') }}"
                                            data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                            data-confirm-yes="document.getElementById('delete-form-{{ $user->id }}').submit();">{{ __('Delete') }}</a>
                                    ...
                                    <a href="#" class="dropdown-item text-sm"
                                        ...
                                        title="{{ __('Reset Password') }}">{{ __('Reset Password') }}
                                    </a>
                        ...
                        <div class="mt-4">
                            <div class="row justify-content-between align-items-center">
                                ...
                                <div class="col-12 text-center pb-2">
                                    <span class="text-dark text-xs">{{ __('Plan Expire : ') }}
                                        {{ !empty($user->plan_expire_date) ? \Auth::user()->dateFormat($user->plan_expire_date) : 'Unlimited' }}</span>
                                </div>
                                <div class="col-6 text-center">
                                    <span
                                        class="d-block text-sm font-weight-bold mb-0">{{ \Auth::user()->countUsers() }}</span>
                                    <span class="d-block text-sm text-muted">{{ __('Users') }}</span>
                                </div>
                                <div class="col-6 text-center">
                                    <span
                                        class="d-block text-sm font-weight-bold mb-0">{{ \Auth::user()->countEmployees() }}</span>
