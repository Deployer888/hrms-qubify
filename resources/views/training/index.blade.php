                        ...
                        <table class="table table-striped mb-0 dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('Cost') }}</th>
                                    @if (Gate::check('Edit Training') || Gate::check('Delete Training') || Gate::check('Show Training') || isset(\Auth::user()->employee->is_team_leader) && \Auth::user()->employee->is_team_leader == 1)
                            ...
                            <tbody class="font-style">
                                @foreach ($trainings as $training)
                                    <tr>
                                        ...
                                        @if( isset(\Auth::user()->employee->is_team_leader) && \Auth::user()->employee->is_team_leader == 1)
                                        <td class="text-right action-btns">
                                            <a href="{{ route('training.show', \Illuminate\Support\Facades\Crypt::encrypt($training->id)) }}"
                                                        class="edit-icon bg-success" data-bs-toggle="tooltip"
                                                        title="{{ __('View Detail') }}"><i
                                            ...
                                            <td class="text-right action-btns">
                                                ...
                                                @can('Edit Training')
                                                    <a href="#" data-url="{{ route('training.edit', $training->id) }}"
                                                        ...
                                                        title="{{ __('Edit ') }}" class="edit-icon"><i
