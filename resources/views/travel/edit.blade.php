    ...
    <form action="{{ route('travel.update', $travel->id) }}" method="post">
        ...
        <div class="row">
            <div class="form-group col-md-12">
                ...
                <select name="employee_id" id="employee_id" class="form-control select2" required>
                    @foreach ($employees as $id => $name)
                        <option value="{{ $id }}" @if ($travel->employee_id == $id) selected @endif>
                            {{ $name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group col-md-12">
                ...
                <textarea name="description" id="description" class="form-control" placeholder="{{ __('Enter Description') }}">{{ $travel->description }}</textarea>
            </div>
            <div class="col-12">
