<div class="row padding-1 p-1">
    <div class="col-md-12">
        ...
        <div class="form-group mb-2 mb20">
            <label for="technology_id" class="form-label">{{ __('Industry Type') }}</label>
            <select name="industry_id" class="form-control @error('technology_id') is-invalid @enderror"
                id="technology_id">
                <option value="">Select Industry Type</option>
                @foreach($types as $technology)
                    <option value="{{ $technology->id }}"
                        {{ $industryList->industry_id == $technology->id ? 'selected' : '' }}>
                        {{ $technology->name }}
                    </option>
                @endforeach
            </select>
